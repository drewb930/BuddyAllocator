...
public:
	...
	void remove(BlockHeader* b) {  // removes a block from the list
		...
		if (curr == b) {
			...
			while (curr->next != NULL && curr->next != b) {
				curr = curr->next;
			}
			curr->next = b->next;
		}
...
private:
	...
	int get_start_index(int size);
	int get_end_index(int size);
	void split(BlockHeader* block, int loc, int target_size);
...
public:
	...
	*/
	...
	BlockHeader* make_block(int size, char* address, int target_size);
	void printlist();
